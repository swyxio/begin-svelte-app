...
export default async function NoobCommentsPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  return (
    <div>
      ...
      {visibleComments.length >= ITEMS_PER_PAGE && (
        <Pagination page={page} basePath="/noobcomments" />
      )}
    </div>
  );
}
